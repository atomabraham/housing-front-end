...
function FormPostProperty() {
  ...
  const [availableTableOptions, setAvailableTableOptions] =
    useState(tableOptions);
  ...
  const handleSelect = (event) => {
    ...
    if (!selectedItems.includes(selectedOption)) {
      ...
      const updatedTableOptions = availableTableOptions.filter(
        (tableOptions) => tableOptions.value !== selectedOption
      );
          ...
          <>
            ...
            >
              <p className="basicInformation" id="basicInformation">
                Détails de la propriétés
              </p>
            </Row>
            <Form.Group className="mb-3">
              <Form.Label className="labelBlockPost" id="labelDescription">
                Description
              </Form.Label>
              <textarea
                name="inputFormPostDescription"
                type="text"
                className="inputFormPost inputFormPostDescription"
                id="inputFormPostDescription"
                placeholder="Donnez une brève description de votre produit"
              />
            </Form.Group>
            <label for="supplement" className="labelBlockPost">
              <b>Agréments</b>
            </label>
            <ul id="selectedItems" className="selected-items">
              {selectedItems.map((item, index) => (
                <li
                  className="optionSelectCreateProp"
            ...
            >
              <option
                disabled
                value="Selectionnez tous les suplément que contient votre propriété"
              >
                Selectionnez tous les suplément que contient votre propriété
              </option>
              {tableOptions.map((option) => (
                <option value={option.value} className="optionFormPost">
                  {option.value}
                </option>
              ))}
            </Form.Select>
          ...
          <>
            ...
            <Row>
              {images.map((image, index) => (
                <Col md="4">
                  <img
                    className="img-thumbnail m-2 backgroundLogo2"
                    key={index}
                    src={URL.createObjectURL(image)}
                    alt={`Image ${index}`}
                  />
                  <svg
                    className="bi bi-x deleteImage"
